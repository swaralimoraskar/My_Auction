...
export const verifyCommissionCron = () => {
  cron.schedule("*/1 * * * *", async () => {
    ...
    const approvredProofs = await PaymentProof.find({ status: "Approved" });
    for (const proof of approvredProofs) {
      try {
        ...
        if (user) {
          ...
          your recent payment has been successfully verified and settled. Thank you for
          promptly providing the necessary proof of payment. Your account has been updated,
